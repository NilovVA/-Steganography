...
def bits_to_string(bits, to_str=False):
    idx = 0
    result = []
    current = ''
    ...
    for bit in bits:
        ...
        if idx % 8 == 0 and current != '':
            idx = 0

            if to_str:
                result.append(decoding_table[int(current, 2)])
            else:
...
whitespace_bits = "00100000"
first_whitespace_found = False
...
for line in container_lines:
    ...
    if container_index == 8 and result_message == '00000000':
        print("Похоже, что в представленной контейнере нет сокрытой фразы")
        exit(1)

    if not first_whitespace_found:
        if len(result_message) % 8 == 0 and result_message[-len(whitespace_bits):] == whitespace_bits:

            first_whitespace_found = True
            hide_len_bits = result_message[:-len(whitespace_bits)]
            hide_len = int(bits_to_string(hide_len_bits))
            print(hide_len)
...
result_message = ''
result_chars_bits = []
hide_message_index = 0
while hide_message_index < hide_len:

    index = container_index + hide_message_index
    if container_lines[index].endswith(' '):
        result_message += '1'
    else:
        result_message += '0'
    hide_message_index += 1

hide_message = bits_to_string(result_message, to_str=True)

output_filepath = "найденная_сокрытая_фраза_" + str(datetime.datetime.now())
output_file = io.open(output_filepath, "w")
output_file.write(hide_message)
output_file.flush()

print("Файл, содержащий найденную фразу, можно найти по пути: " + output_filepath)